...
const MypageModal = ({isOpen, onClose, logout, userDatas}) => {
  ...
  return (
    ...
    <div className={`modal ${isOpen ? 'block' : 'hidden'} absolute inset-0 h-full w-full bg-transparent z-30 `}>
      ...
      <animated.div
      style={{ ...modalAnimation, top: '2%', height: '30%'}}
      className={`modal-content absolute w-1/2 left-1/4 top-0 transform -translate-x-1/2 -translate-y-1/2 bg-black rounded-3xl`}>
        <div className={`absolute bg-black w-full h-full opacity-75 ${isDeleteModalOpen ? 'block' : 'hidden'}`}></div>
        <div className='w-full h-full justify-between flex flex-col'>
          ...
          <div className='mb-2 flex flex-col'> 
            ...
            <div className='w-full flex flex-col gap-2 border-2 p-2 rounded-xl'>
            {currentProducts.map((product) => (
              <div key={product.id} className='rounded-lg border-2 text-white px-5 flex justify-center'>
                <div className='flex w-3/4'>
                  <div className='flex-1 w-1/6 p-3'>
                    <img className='w-3/5' src='img/상의.png' alt={`Product ${product.id}`} />
                  </div>
                  <div className='flex-1 flex flex-col gap-2 justify-center font-KIMM_Bold my-1 '>
                    ...
                    <div>
                      <label>PRICE ▷ </label>
                      <label className='text-orange-400'>{product.sellPrice}</label>
                    </div>
                    <div>
                      <label>DATE ▷ </label>
          ...
          <div className='flex gap-2 font-KIMM_Bold items-end justify-end mr-1'>
              <div className='text-black border-2 rounded-md px-3 py-1 bg-white cursor-pointer hover:bg-black hover:text-white'>MODIFY</div>
